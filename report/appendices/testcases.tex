...
%===================
\label{sec:testcases}
This section introduces the test cases we ran during the different sprints of the project. All of the test cases follow the template mentioned in the test plan on \autoref{tab:testcase}.


%-----------------------
\section{Sprint 1 Tests}
%-----------------------
In this sprint we ran the test cases shown in \autoref{tab:TID01}-\ref{tab:TID07}. The test cases made for this sprint covers most of the extremely simple functionality needed in order to have a working utility. 


%-----------------------
\section{Sprint 2 Tests}
%-----------------------
In this sprint we ran the test cases shown in  \autoref{tab:TID08}-\ref{tab:TID14}. These are tests to see that the basic functionalities of the utility are in place. 


%------------------------------
\section{Sprint 3 Tests}
%-----------------------------
In this sprint we ran the test cases shown in \autoref{tab:TID15}-\ref{tab:TID23}. These are tests to see that the utility is working properly even with the most advanced features.


%----------------------
\section{Sprint 4 Tests}
%----------------------
In this sprint we ran the test cases shown in \autoref{tab:TID26}-\ref{tab:TID48}. These are tests to see that the utility has been fixed in accordance to the  extra requirements made by the customer in sprint 3 as well as the final touches that were needed in order to get the utility to work in a real environment. This section also includes the test cases made for the non-functional requirements.
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	Prerequisites & The utility must have been installed on the system and there needs to exist a header, config and pcap file associated with this test \\
	Feature & Test that the \gls{utility} is able to support \Gls{c}-\gls{header} files with \glspl{struct} that have other \gls{struct} \glspl{member}  and display it properly in \Gls{wireshark}\\
	\midrule
	...
	& 5. The \gls{struct} \glspl{member} within \glspl{struct} should be displayed as subtrees, indicated by having a clickable ''+'' box to the left which when clicked, displays the contents of the given \gls{struct}\\
	\bottomrule
\end{tabularx}}
\end{table}
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	Tester & Erik Bergersen \\
	Prerequisites & The \gls{utility} must have been installed on the system and there needs to exist a header, config and pcap file associated with this test \\
	Feature & Test that the \gls{utility} is able to support \Gls{c}-\gls{header} files with \gls{trailers} \\
	\midrule
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	Feature &  Test that the \gls{utility} is able to generate \glspl{dissector} for all \gls{header}-files in a folder, with configuration\\
	\midrule
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	\midrule
	\multirow{2}{*}{Expected result}  & 2. The program should provide the user with some text expressing the success of generating a dissector for each of the structs inside the header file. \\
	& 5. The different \glspl{struct} should be displayed as having the right \gls{struct} \glspl{member} according to the \Gls{c}-\gls{header} definitions. For example, the \gls{member} defined surrounded by \texttt{\gls{ifdef} \_WIN32 ... \#endif} directives should be displayed correctly in WIN32 \gls{struct}. \\
	\bottomrule
\end{tabularx}}
\end{table}
...
\begin{tabularx}{1.2\textwidth}{l X} 
	...
	Tester & Erik Bergersen \\
	Prerequisites & There have to exist a \gls{header}-file, config-file and pcap-file to do the test. \\
	Feature & Generate \glspl{dissector} on platforms with different alignment \\
	\midrule
	\multirow{5}{*}{Execution} & 1. Feed the \gls{utility} with the name of the \Gls{c}-\gls{header} file associated with this test\\
	& 2. Read the output \\
	& 3. Move the resulting \gls{dissector} into the plugins folder in the personal configuration folder of \Gls{wireshark}\\
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	\midrule
	\multirow{7}{*}{Execution} & 1. Feed the \gls{utility} with any \Gls{c}-\gls{header} file and it's configuration\\
	& 2.  Read the output\\
	& 3. Copy the resulting \glspl{dissector} into the plugins folder of the personal configuration in \Gls{wireshark}\\
	& 4. Run \Gls{wireshark}\\
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	\midrule
	\multirow{5}{*}{Execution} & 1. Feed the \gls{utility} with a \Gls{c}-\gls{header} file that has pointers, and it's configuration file\\
	& 2.  Read the output\\
	& 3. Copy the resulting \glspl{dissector} into the plugins folder of the personal configuration in \Gls{wireshark}\\
	& 4. Run \Gls{wireshark} with the \gls{pcap-file} associated with this test\\
	& 5. Inspect the different \glspl{packet}, their \glspl{struct} and \gls{member} values\\
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	\midrule
	\multirow{2}{*}{Execution} & 1. Feed the \gls{utility} with a \Gls{c}-\gls{header} file that has an \gls{array} of \glspl{enum}, and it's configuration file\\
	& 2.  Read the output\\
	\midrule
	\multirow{1}{*}{Expected result}
	& 2. The user should be presented with some text expressing the success of generating \glspl{dissector}\\
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	\midrule
	\multirow{5}{*}{Execution} & 1. Feed the \gls{utility} with a \Gls{c}-\gls{header} file, it's configuration file and a \gls{define} directive\\
	& 2.  Read the output\\
	& 3. Copy the resulting \glspl{dissector} into the plugins folder of the personal configuration in \Gls{wireshark}\\
	& 4. Run \Gls{wireshark} with the \gls{pcap-file} associated with this test\\
	& 5. Inspect the different \glspl{packet}, their \glspl{struct} and \gls{member} values\\
