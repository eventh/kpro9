...
\begin{description}
	\item[\Gls{c} \gls{preprocessor}] To fulfill all the requirements we might need to
		...
		a huge undertaking.
	\item[Platforms] \gls{asparc} platform that are required for the program are not
...
the same application. If we had to use different \Gls{uml} applications we could have problems
...
from \Gls{ntnu}, and are listed in \autoref{tab:plan:devs}. The team is assigned two
...
\begin{longtable}{l p{9cm}}
	...
	Consequences & \Gls{h}: The team waste time on functionality the customer did not want. The completed product does not do what the customer wanted. \\
	Probability & \Gls{m} \\ 
