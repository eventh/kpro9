...
Waterfall\footnote{\url{http://www.waterfall2006.com/}} is a software development methodology based on sequential phases.
...
%-----------------------
\label{sec:pre:scrum}
\Gls{scrum} \footnote{\url{http://members.cox.net/risingl1/Articles/IEEEScrum.pdf}} is an agile development methodology based on the philosophy that it is
...
parse. \Gls{clang} provides bindings for \Gls{python} so it can be used as a \gls{library},
...
\subsection{YAML Ain't Markup Language}
%--------------------------------------
\label{sec:pre:yaml}
...
is a \Gls{python} module used for managing user-editable configuration files. The
...
\noindent Currently, ConfigObj module only exists for \Gls{python} up to version
2.7. It is under the \Gls{bsd} license.
...
section. In \autoref{sec:pre:testchoice}, we describe which one we selected and why.

\subsection{py.test}
%-------------------
py.test\footnote{\url{http://pytest.org/latest/}} is a mature, full-featured testing
tool. It runs on \Gls{python} 2.4-3.2, PyPy and Jython-2.5.1 interpreters on both
...
unit test support that can help finding errors and it has integrated version control
support, including git, which makes it easy to synchronize with a remote
repository. The most used functions are also paired with keyboard shortcuts.
...
not already experienced with \gls{vim} to use it.
...
require us to modify these grammars to suit our needs, and \gls{antlr}'s lack of
...
For \Gls{c} \gls{preprocessor} we have selected to use a tool for Windows that comes with
\gls{pycparser}, on Mac we will use the one that comes with XCode, and on other
platforms we will either use \Gls{agcc} or tools that comes with the platform. If we
...
\begin{tabularx}{\textwidth}{X X X X}
	\toprule
	& \Gls{yaml} & configparser & ConfigObj \\
	\midrule
	Advantages &
	+Simplicity\newline +Flexibility &
	+Easy to use &
	...
	\Gls{python} \gls{branch} & 2.7 and 3.3 & 2.7 and 3.3 & 2.7 \\
	License & \Gls{mit} & PSF L & \Gls{bsd}-new \\
	\bottomrule
\end{tabularx}
\end{table}
...
Our \gls{utility} will need to take \Gls{c} \gls{header} files as input, search through them to
...
When we decided which license to use, we had to consider the licenses of the \glspl{library} and tools we depend upon.
