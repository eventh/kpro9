...
used for example in \Gls{Sun RPC} and \GLS{corba}. Since idl2wrs takes input in a different
...
\subsection{Asn2wrs}
%-------------------
A tool for generating \Gls{wireshark} \glspl{dissector} from \GLS{asn1} \glspl{protocol}. It requires
four input files: an \GLS{asn1} \gls{protocol} description, a configuration file and two
...
solution as it would require them to rewrite their \Gls{c} \glspl{struct} to \GLS{asn1}
...
interface with \Gls{wireshark} through a simple API.
...
several pure \Gls{python} \glspl{library} for creating \glspl{lexer} and \glspl{parser}, like \GLS{ply},
...
tools like \GLS{gcc} and \Gls{clang} to \Gls{python}-only \glspl{library} like \GLS{ply} and \gls{pycparser}. The
...
we are facing: parsing \Gls{c}, \Gls{c} \gls{preprocessor} step and parsing \GLS{asn1}. These grammars
configure ANTLR to create \Gls{java} \glspl{lexer} and \glspl{parser} which reads and validates
inputted source code files.

\subsection{\GLS{ply}}
%---------------
\GLS{ply}\footnote{\url{http://www.dabeaz.com/ply/}} is a \Gls{python} alternative to the
...
cppheaderparser depends on \GLS{ply}. These are described later in this section.
...
specification. \gls{pycparser} requires \GLS{ply} to work.
...
\begin{wrapfigure}{r}{1.5cm}
	...
	\vspace{-20pt}
\end{wrapfigure}
GNU Compiler Collection\footnote{\url{http://gcc.gnu.org/}} (\GLS{gcc}) is a
...
a custom \gls{parser} or use something like \GLS{GCC-XML} and an \GLS{xml} \gls{parser}.
...
\Gls{c++}, \Gls{Objective-C} and \Gls{Objective-C++}, written in \Gls{c++}. \Gls{clang} differ from \GLS{gcc} as it
behaves as a \gls{library} rather than an external tool, but for \Gls{java} we will have to
use it like \GLS{gcc} because there are no \Gls{java}-\Gls{clang} bindings. It supports
outputting the \gls{AST} as \GLS{xml}, which our \gls{utility} then will need to
parse. \Gls{clang} provides bindings for \Gls{python} so there it can be used as a \gls{library},
but its main drawback is, like \GLS{gcc}, a lack of flexibility. \Gls{clang} is a part of
the LLVM toolkit.
...
\subsection{\GLS{asn1}}
%-----------------
Abstract Syntax Notation One (\GLS{asn1}) is a notation for describing data
structures, the standard is defined by ISO 8824. \GLS{asn1} is used for
...
of  \GLS{asn1}, but neither support the 3.x branch of \Gls{python}, which we require.
...
\begin{wrapfigure}{r}{3cm}
	...
	\vspace{-30pt}
\end{wrapfigure}
YAML\footnote{\url{http://yaml.org/}} (YAML Ain't Markup Language) is a \gls{data serialization} format.
...
PyYAML\footnote{\url{http://pyyaml.org/}} is a YAML \gls{parser} for the \Gls{python}
programming language, and it is available for both the 2.x and 3.x branch of
\Gls{python}. It is licensed under the MIT license.
...
\Gls{python} 2.x and 3.x branches.
...
results in plain text, html and \GLS{xml}.
...
is a cross platform, proprietary IDE for \Gls{python}. It has good support
...
is a Windows only, open source IDE for \Gls{python}. It has support for
...
members are best acquainted with \Gls{Eclipse}. It will be the recommended IDE for
...
third is using a toolkit \gls{parser} like \hyperref[sec:pre:gcc]{\GLS{gcc}} and
...
a large dependency which our customer want to avoid if possible. \GLS{gcc} and \Gls{clang}
...
specification. \gls{pycparser} depends on \GLS{ply}, so our \gls{utility} will also depend on it.
...
platforms we will either use \GLS{gcc} or tools which comes with the platform. If we
need to modify a \Gls{c} \gls{preprocessor} we might use \GLS{ply}'s incomplete \Gls{c} \gls{preprocessor}.
...
\begin{tabularx}{1.1\textwidth}{X X X X X}
	\toprule
	& YAML & configparser & ConfigObj & \GLS{asn1} \\
	\midrule
	Advantages &
	+Simplicity\newline +Flexibility &
	+Easy to use &
	...
	\Gls{python} branch & 2.7 and 3.3 & 2.7 and 3.3 & 2.7 & 2.7 \\
...
frameworks was reduced. Therefore, although \GLS{asn1} and ConfigObj are very
...
the \Gls{c} \glspl{header}. YAML has only two minor disadvantages we should be aware of.
...
\begin{tabular}{l l l}
	...
	\GLS{ply} & 3.4 & Latest stable version \\
...
whichever license we feel is most suited. They suggested GNU GPL as \Gls{wireshark}
...
\begin{tabular}{l l}
	\toprule
	\Gls{wireshark} & GNU GPL \\
	\GLS{ply} & BSD-new \\
	\gls{pycparser} & BSD-new \\
	pyYAML & MIT \\
	\midrule
	Our \gls{utility} & BSD-new \\
	\bottomrule
\end{tabular}
\vspace{-10pt}
\end{table}
...
the \Gls{c} \gls{preprocessor} in \GLS{ply} and the \gls{pycparser} \gls{library}, which made us consider
