...
\begin{description}
	...
	\item[FR2] The requirement specify what the dissector shall support, to be 
		able to be display the packet correctly in Wireshark. 
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	Element & Description\\
	\midrule
	Use case & \includegraphics[scale=1]{./planning/img/usecase1} \\
	Brief description &  \\
	Flow of Events &  \\
	Preconditions & Filter and search \\
	\addlinespace
	Use case & Filter and search \\
	\bottomrule
\end{tabularx}}
\end{table}
...
Brief Description A brief description of the role and purpose of the use
case.
Flow of Events A textual description of what the system does in regard
to the use case (not how specic problems are solved by
the system). The description is understandable by the
customer.
Special Requirements A textual description that collects all requirements, such
as non-functional requirements, on the use case, that are
not considered in the use-case model, but that need to
be taken care of during design or implementation.
Preconditions A textual description that denes a constraint on the
system when the use case may start.
Postconditions A textual description that denes a constraint on the
system when the use cases have terminated.
Extension points A list of locations within the ow of events of the use
case at which additional 
%\begin{figure}[htbp]
%	\center
%	\includegraphics[width=0.8\textwidth]{./planning/img/developer}
%	\caption{Use Case Diagram: Developer\label{fig:req:ucdev}}
