...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	What & The administrator wants to generate \glspl{dissector} that contain \glspl{struct} with \glspl{union} as \glspl{member}. \\
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	\midrule
	ID & US30 \\
	Requirement & FR5-C: Generate \glspl{dissector} that support both little and big \gls{endian} platforms \\
	What & The administrator wants \glspl{dissector} that handle both big and little \gls{endian} encoding. \\
	...
	The \gls{dissector} module shall have functionality that generates \Gls{lua} code depending on \gls{endianness}, and the different Field classes must use this function when adding 
	nodes to the \Gls{wireshark} tree. \\
	Result & \Glspl{dissector} can be created with platform specific \gls{endian}. \\
	\midrule
	ID & US31 \\
	Requirement & FR5-D: Generate \glspl{dissector} that support different sizes depending on platforms \\
	...
	with \Gls{c} \gls{preprocessor} directives for each platform. These files should define platform-specific macros that emulates parsing on the specific platform. 
	The \gls{dissector} module then create different \glspl{dissector} for each message on each platform, and a mapping is added inside the master \Gls{lua} file that maps message id and
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	This section gives a good description of how to add or remove support for a platform in the configuration, in order for the administrator to understand how to do
	this after reading the section. \\
	Result & The administrator is able to configure the \gls{utility} to add or remove platform support from the generated \glspl{dissector}. \\
	\midrule
	ID & US36 \\
...
\begin{tabularx}{1.2\textwidth}{l X}
	...
	What & The administrator wants to generate \glspl{dissector} that are different depending on platform. \\
	How & The \gls{dissector} module will use the configured flags from the config module to modify the generated \Gls{lua} \glspl{dissector}, to have the right memory structure alignment and \gls{endianness}.
	The \gls{dissector} module shall have a function that calculates the offset for each field to align it, and functions which creates fields for specific \gls{endianness}. \\
...
\begin{itemize}
	\item TID15 - Supporting \gls{batch mode} of \Gls{c} \gls{header} and configuration files
	\item TID16 - Supporting custom \Gls{lua} configuration
	\item TID17 - Supporting \glspl{union} 
	\item TID18 - Supporting filter and search in \Gls{wireshark} 
	\item TID19 - Supporting WIN32, \_WIN64, \_\gls{asparc} etc 
	\item TID20 -  Supporting the use of flags specifying platforms to display \gls{member} values correctly 
	\item TID21 - Supporting platforms with different \glspl{endian} 
	\item TID22 - Supporting alignments 
	\item TID 23 - Handling \Gls{lua} keywords 
	\item TID24 - Sprint 3 functionality test 
	\item TID25 - Sprint 3 requirements test
\end{itemize}
...
The results from the tests are listed in \autoref{tab:sp3:testres1} and \autoref{tab:sp3:testres2}.

\begin{table}[htbp] \footnotesize \center
\caption{Sprint 1 test results part 1\label{tab:sp3:testres1}}
\noindent\makebox[\textwidth]{%
...
\begin{table}[htbp] \footnotesize \center
\caption{Sprint 1 test results part 2\label{tab:sp3:testres2}}
\noindent\makebox[\textwidth]{%
\begin{tabularx}{\textwidth}{l X}
	\toprule
	Header & Description \\
