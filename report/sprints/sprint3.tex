...
\item In the sprint backlog we will have work items for every task that needs to be done throughout the sprint, including writing minutes, doing documentation, implementation and so on. Now we do not assign responsibilities for all the items in the planning meeting, we rather only give one responsibility to each team member at a time. The rest of the items will be unassigned. At each stand-up meeting we pick a task and must be done with it at the next meeting. This will ensure efficiency and the work done by others are easier to check. It will give us a better work balance, as no team member can gap over to many tasks and leave none for the others.
\end{itemize}
