...
\begin{itemize}
	...
	\item The existing features have to be extensively tested and documented before the project is over, leaving little time for other work. Testing and bugfixing will usually not result in new design, except minor changes in the class diagram.
\end{itemize}

\subsection{System overview}
%-----------------------
The utility lacked the last, but also most essential implementation at the start of fourth sprint; handling of the customer's source code.

%class diagram
The addition of new requirements made a re-factoring necessary:
\begin{itemize}
\item The need for custom handling of C preprocessor steps on C header files have become so comprehensive, that we have decided to create our own module for it.
...
This section covers the the added functionality and the most important fixes 
in this sprint.
...
include all their header-files in the code file, and they only are going to 
generate Lua dissectors from header-file. The reason they only will genrate 
from header-files is that some of the code is c++, and the size of all the 
...
to be able to parse header-files that depend on other header-files. To be able 
to solve this problem, the utility have to find the includes that the 
header-file depends on, and include them in the correct order, because the 
included header-files can be dependent on each other.
...
%--------------------
To improve the performance of the Lua dissectors in Wireshark, it was 
necessary to change how the different platforms was handled in the Lua 
dissector. Until this sprint the utility generated one dissector table for 
each platform. To make the dissectors more efficient in Wireshark, it was 
changed to only have one dissector table for all the dissectors, and call 
different function for each of the platform. To change this the dissector 
module in CSjark was modified to generate the dissectors correctly. By fixing 
...
%--------------------
After adding support for complex arrays in sprint 3, some bugs occured in the 
proto fields for arrays. The name for these proto field was fixed, so it is 
possible to filter and search for values in arrays. 

\subsection{CLI Support for Include Folders}
%--------------------
Support for specifing include folders was implemented in this sprint. This is 
given as an argument to the CLI with  \emph{-I} or \emph{-Includes} followed 
by the folders to include. The folders included will be added as an argument 
to the preprocessor, so the preprocessor can search for files in these 
folders, when a file is given in a include directive(\#include). 

\subsection{CLI Support for C Macro Definitions}
%--------------------
The utility supports C Macro definitions as arguments from the CLI. A macro 
definition is also known as \emph{\#define}. This feature was added since it 
should be possible to add macro definitions to the preprosessor, instead of 
modifing several header-files. During a run of the utility with macro 
definitions specified, the definitions will be given for all the parsed 
header-files.

\subsection{Support Multiple Dissector ID }
%--------------------
...
%--------------------
Header-files that the utility parses, may have have nested struct that is not 
defined in any other header file. To make the it possible to generate a 
dissector for this case, it must be possible to specify the size of the struct 
in a configuration file. When the sizes is specified it will be possible to 
generate a struct that can display the defined members of the struct correctly 
in Wireshark, for the parts that are not defined will only the hex value be 
displayed. 

This feature is added as a possible way to solve include dependencies that our 
utility is not able to solve. The user of the utility will get an error 
message when the utility is not able to find the include dependencies, and the 
user may add the size of struct to be able to generate a dissector for struct. 
An example of configuration of size for a struct, is shown in 
...
the handling of custom lua files. This feature was that it should be possible 
to add new proto fields to the dissector protocol, with correct offset value 
and correct Lua variable. To be able to do this, it was necessary to add a 
variables for offset and value in the conformance file. Use of the Lua 
variable and offset value is only possible in the functionality part of the 
Lua dissector. \autoref{code:customcnfsp4} is an example of how value and 
offset can be used, and \autoref{code:customluasp4} shows the result in the Lua 
dissector.
...
displayed in Wireshark, with correct sizes for different platforms.

TODO: Remove this subsection if not implemented
...
\subsection{CLI Support to Exclude Files or Folders}
%--------------------
TODO
