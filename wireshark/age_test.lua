...
function proto_age_test.dissector(buffer, pinfo, tree)
	...
	local name_value = subtree:add(f.name,buffer(0,10):string())
	local age_value = subtree:add(f.age,buffer(10,4):int32())
	local age_min = 0.0
	if (age_min > age_value) then
		age_value:append_text("INVALID")
	end
	local age_max = 100.0
	if (age_max < age_value) then
		age_value:append_text("INVALID")
	end
end
