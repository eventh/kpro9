...
void hexdump (unsigned char *data, unsigned long length)
{
   int i = 0;
   FILE *fp;
   fp=fopen("out.hexdump", "w");
   fprintf(fp,"0000 01 01 02 9A ");
   for (i = 0; i < length; i++) {
	  fprintf(fp,"%02x ", data[i]);
      /* printf ("%02x ", data[i]); */
      /* if (((i+1) % (80/3)) == 0) printf ("\n"); */
   }
   fclose(fp);
}
...
int main(int argc, char *argv[])
{
   ...
   hexdump((unsigned char *)xc, sizeof (struct xcon));
}
